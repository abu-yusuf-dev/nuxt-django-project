...
export default {
  ...
  watch: {
    state(value) {
      ...
      this.$emit('onValidate', this.response);
      this.maxInput = this.response.number.length
      console.log('this.response: ', this.maxInput)
      console.log('this.response: ', this.formattedResult.length)
    },
    value() {
      this.phone = this.value;
    },
  ...
  methods: {
    ...
    onInput() {
      if (this.maxInput > this.formattedResult.length){
        this.response.isValid = false;
        this.$emit('input', this.response.isValid);
      }
