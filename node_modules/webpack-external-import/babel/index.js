...
const validMagicStrings = ['webpackMode', // 'webpackMagicChunkName' gets dealt with current implementation & naming/renaming strategy
...
function getMagicWebpackComments(importArgNode) {
  ...
  if (leadingComments && leadingComments.length) {
    leadingComments.forEach(comment => {
      try {
        const validMagicString = validMagicStrings.filter(str => new RegExp(`${str}\\w*:`).test(comment.value)); // keep this comment if we found a match
  ...
  return {
    ...
    visitor: {
          ...
          comments.forEach(comment => {
            if (comment && comment.value) {
              ...
              if (splitComment[0] === 'externalize') {
                const functionToExport = splitComment[1].trim(); // props use this instead of module proto to actually export out the one single function
        ...
        (0, _babelTraverse.default)(parentPath.node, {
          ArrowFunctionExpression(path) {
            ...
            if (path.isArrowFunctionExpression()) {
              ...
              const injectedDepencency = Array.from(moduleMaps).map(moduleName => {
                return t.assignmentExpression('=', t.identifier(`const ${moduleName}`), t.identifier(`__webpack_require__("${moduleName}")`));
              });
      ...
      Import(p) {
        ...
        if (importArgNode.value) Object.assign(importWhitelist, {
          [importArgNode.value]: null
        });
        const universalImport = getImport(p, IMPORT_UNIVERSAL_DEFAULT); // if being used in an await statement, return load() promise
        ...
        t.isAwaitExpression(p.parentPath.parentPath.node) // await not transformed already
            ...
            const func = t.callExpression(universalImport, [loadOption(t, loadTemplate, p, importArgNode).value, t.booleanLiteral(false)]);
