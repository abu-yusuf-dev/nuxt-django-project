...
class URLImportPlugin {
  constructor(opts) {
    ...
    this.opts = Object.assign({
      ...
      generate: null,
      sort: null,
      serialize: manifest => `if(!window.entryManifest) {window.entryManifest = {}}; window.entryManifest["${opts.manifestName}"] = ${JSON.stringify(manifest, null, 2)}`
    }, opts || {});
  }
  ...
  apply(compiler) {
    ...
    const emit = (compilation, compileCallback) => {
      ...
      }, files), []); // module assets don't show up in assetsByChunkName.
      ...
      files = files.filter(file => {
        ...
        const isUpdateChunk = file.path.includes('hot-update'); // Don't add manifest from another instance
      ...
      if (isLastEmit) {
        const cleanedManifest = Object.entries(manifest).reduce((acc, [key, asset]) => {
    ...
    if (compiler.hooks) {
      ...
      compiler.hooks.compilation.tap(pluginOptions, ({
        hooks
      }) => {
